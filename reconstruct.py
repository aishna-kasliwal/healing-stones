...
OUTPUT_DIR = "./output"
MAX_POINTS = 100000      # more points = better features
MIN_FITNESS = 0.03       # lower threshold = catch more matches
MAX_ITER_ICP = 200       # more ICP iterations = finer alignment
...
def boundary_sample(pcd, n_points=100000, radius=10.0):
    pts = np.asarray(pcd.points)
    if len(pts) == 0:
        return pcd
...
def ransac_reg(src, dst, src_feat, dst_feat, voxel_size):
    dist = voxel_size * 1.5
    result = o3d.pipelines.registration.registration_ransac_based_on_feature_matching(
        src, dst, src_feat, dst_feat,
        mutual_filter=True,
...
def multiscale_register(src, dst):
    ...
    # coarse to fine — more scales overnight
    for voxel_size in [15.0, 10.0, 7.0, 5.0, 3.0, 2.0]:
        try:
            src_d, src_f = preprocess(src, voxel_size)
            dst_d, dst_f = preprocess(dst, voxel_size)
            ransac = ransac_reg(src_d, dst_d, src_f, dst_f, voxel_size)

            # refine with ICP at current scale
            icp = icp_refine(src_d, dst_d, ransac.transformation, voxel_size)

            # if good match found, do extra fine ICP
            if icp.fitness > 0.3:
                fine_icp = icp_refine(src, dst, icp.transformation, voxel_size * 0.5)
                if fine_icp.fitness > icp.fitness:
                    icp = fine_icp
...
def build_pose_graph(clouds):
    ...
    print(f"\ncomputing {total} pairwise registrations (overnight mode)...")

    for i, j in combinations(range(n), 2):
        name_i, pcd_i = clouds[i]
        name_j, pcd_j = clouds[j]

        T, fitness, rmse = multiscale_register(pcd_i, pcd_j)
        ...
        # save progress every 20 pairs in case of crash
        if done % 20 == 0:
            np.save(os.path.join(OUTPUT_DIR, "fitness_matrix_checkpoint.npy"), fitness_matrix)
            print(f"  [checkpoint saved at {done}/{total}]")

    return pose_graph, fitness_matrix, rmse_matrix
...
def plot_before_after(clouds, pose_graph, save_path):
    ...
    plt.suptitle("Mayan Stele — Before vs After Registration", fontsize=14, fontweight="bold")
    plt.tight_layout()
...
def plot_fitness_matrix(fitness_matrix, names, save_path):
    ...
    ax.set_title("Pairwise Registration Fitness (6-scale RANSAC+ICP)")
    plt.tight_layout()
    plt.savefig(save_path, dpi=150)
    plt.close()
    print(f"saved -> {save_path}")
...
def plot_graph(fitness_matrix, names, save_path):
    ...
    ax.set_title("Fragment Connectivity Graph (overnight run)")
    ax.axis("off")
    plt.tight_layout()
    plt.savefig(save_path, dpi=150)
    plt.close()
...
def main():
    ...
    if n == 0:
        print("no fragments loaded, check path")
        return

    pose_graph, fitness_matrix, rmse_matrix = build_pose_graph(clouds)
    ...
    out_ply = os.path.join(OUTPUT_DIR, "reconstructed_stele.ply")
